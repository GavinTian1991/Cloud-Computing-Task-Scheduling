...
public class TSImplement {
	...
	public static void main(String[] args) {
		...
		try {
			...
			for (int k=1;k<=10;k++){
			    ...
			    Datacenter datacenter0 = createDatacenter("Datacenter_0",num);
				Datacenter datacenter1 = createDatacenter("Datacenter_1",num);
				...
				vmlist = createVM(brokerId,num);
				vmlist1 = createVM(brokerId1,num);
				
				
				cloudletList = createCloudlet(brokerId,100*k);
				cloudletList1 = createCloudlet(brokerId1,100*k);
				//Log.printLine(cloudletList1.size());
				...
				double max1 = 0;
				double sum1 = 0;
				double dtaskNum1 = k * 100;
				for (int j=0;j<100*k;j++){
					sum1 += newList1.get(j).getFinishTime();
				...
				double aver1 = sum1 / dtaskNum1;
				double di1 = (max1 - min1) / aver1;
	...
	private static DatacenterBroker createBroker(){
		...
		try {
			//int m, iteration, double Q, double alpha, double beta, double gamma, double rho
			broker = new DatacenterBroker("Broker",40,100,1,2,1,4,0.4);
		} catch (Exception e) {
			e.printStackTrace();
			return null;
		}
	...
	private static DatacenterBroker1 createBroker1(){
		...
		//( name, m, iteration, initialPheromone, Q,  alpha,  beta,  rho)
		DatacenterBroker1 broker1 = null;
		try {
			broker1 = new DatacenterBroker1("Broker1",40,100,1,1,2,1,0.4);
		} catch (Exception e) {
			e.printStackTrace();
			return null;
		}
